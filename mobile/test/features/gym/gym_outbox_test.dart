...
void main() {
  ...
  group('overlay', () {
    ...
    test('a workout started offline shows on the overview and counts', () {
      final overlaid = overlayGym(empty, [
        CreateSession(session: _session('w1'), queuedAt: _at),
      ]);

      expect(overlaid.sessions.single.id, 'w1');
      expect(overlaid.totalSessions, 1);
    });

    test('unsent sets show on the workout the server already knows', () {
      final overview = empty.copyWith(sessions: [_session('server-1')]);

      final overlaid = overlayGym(overview, [
        SaveSession(
          sessionId: 'server-1',
          write: _write(
            notes: 'Felt strong',
            sets: const [GymSetWrite(weight: 75, reps: 10)],
          ),
          queuedAt: _at,
        ),
      ]);

      expect(overlaid.sessions.single.notes, 'Felt strong');
      expect(overlaid.sessions.single.completedSetCount, 1);
    });

    test('reopening an unsent workout gives back what was typed into it', () {
      final restored = overlayGymSession(null, [
        CreateSession(session: _session('w1'), queuedAt: _at),
        SaveSession(
          sessionId: 'w1',
          write: _write(sets: const [GymSetWrite(weight: 75, reps: 10)]),
      ...
      expect(restored?.exercises.single.sets.single.reps, 10);
    });

    test('a gym added offline is pickable', () {
      final overlaid = overlayGym(empty, [
        CreateLocation(
          location: const GymLocation(
            id: 'g1',
      ...
      expect(overlaid.locations.single.name, 'Home');
    });
  });
