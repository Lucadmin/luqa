...
class ErrorResponse {
  ...
  /// Clones this instance of [ErrorResponse] and returns a new one where some of the
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'error'),
            'Required key "ErrorResponse[error]" is missing from JSON.');
        assert(json[r'error'] != null,
            'Required key "ErrorResponse[error]" has a null value in JSON.');
        return true;
      }());

      return ErrorResponse(
        error: ErrorDetail.fromJson(json[r'error'])!,
      );
    }
    return null;
  }
