...
class CreateHabitRequest {
  /// Returns a new [CreateHabitRequest] instance.
  CreateHabitRequest({
  ...
  /// Preferred identity for a habit minted offline. Honoured when it is free; a repeat of one already here answers with that habit rather than making a second.
  ///
  ...
  @override
  String toString() =>
      'CreateHabitRequest[id=$id, name=$name, icon=$icon, color=$color, goalType=$goalType, goalPeriod=$goalPeriod, targetCount=$targetCount, targetSeconds=$targetSeconds, categoryId=$categoryId, scheduleType=$scheduleType, weekdays=$weekdays, weekInterval=$weekInterval, intervalDays=$intervalDays, intervalFromLastDone=$intervalFromLastDone, timesPerPeriod=$timesPerPeriod, anchorDate=$anchorDate, dates=$dates, excludedDates=$excludedDates]';
  ...
  /// Clones this instance of [CreateHabitRequest] and returns a new one where some of the
  /// properties have changed.
  CreateHabitRequest copyWith({
    Optional<String?>? id,
    String? name,
  ...
  /// Returns a new [CreateHabitRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreateHabitRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'),
            'Required key "CreateHabitRequest[name]" is missing from JSON.');
        assert(json[r'name'] != null,
            'Required key "CreateHabitRequest[name]" has a null value in JSON.');
        return true;
      }());
  ...
  static List<CreateHabitRequest> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <CreateHabitRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreateHabitRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreateHabitRequest> mapFromJson(dynamic json) {
    final map = <String, CreateHabitRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateHabitRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreateHabitRequest-objects as value to a dart map
  static Map<String, List<CreateHabitRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final map = <String, List<CreateHabitRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = CreateHabitRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
